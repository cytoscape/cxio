...
public class EdgeAttributesFragmentReaderETest {
	...
	public void test() throws IOException {
		// Verify that a list of "property_of" is not allowed
		try {
			final String multiplePO = "[" + TestUtil.NUMBER_VERIFICATION
					+ ",{\"edgeAttributes\":[{\"po\":\"0\",\"n\":\"name1\",\"v\":\"value\"}]},"
					+ "{\"edgeAttributes\":[{\"po\":[\"0\",\"1\",\"2\"],\"n\":\"name4\",\"v\":[\"1\",\"2\"],\"d\":\"list_of_short\",\"s\":\"1234\"}]}"
					+ "]";
			final CxElementReader badReader = CxElementReader.createInstance(multiplePO, CxioUtil.getAllAvailableAspectFragmentReaders());
		CxElementReader.parseAsMap(badReader);
			fail("Multiple property_of were allowed, but should not be");
		} catch (Exception IllegalArgumentException) {
			// Should have failed ... this is not allowed
		} catch (Throwable t) {
			fail("Unexpected exception: " + t);
		}

		// Verify other edge attribute behaviors
		final String t0 = "[" + TestUtil.NUMBER_VERIFICATION
				+ ",{\"edgeAttributes\":[{\"po\":\"0\",\"n\":\"name1\",\"v\":\"value\"}]},"
				+ "{\"edgeAttributes\":[{\"po\":\"1\",\"n\":\"name2\",\"v\":\"12\",\"d\":\"integer\"}]},"
				+ "{\"edgeAttributes\":[{\"po\":\"2\",\"n\":\"name3\",\"v\":\"true\",\"d\":\"boolean\"}]},"
				+ "{\"edgeAttributes\":[{\"po\":\"3\",\"n\":\"name4\",\"v\":[\"1\",\"2\"],\"d\":\"list_of_short\",\"s\":\"1234\"}]}"
				+ "]";
		...
		assertTrue("failed to parse " + EdgeAttributesElement.ASPECT_NAME + " aspect",
				r0.containsKey(EdgeAttributesElement.ASPECT_NAME));
		assertFalse("failed to parse " + EdgeAttributesElement.ASPECT_NAME + " aspect",
				r0.get(EdgeAttributesElement.ASPECT_NAME).isEmpty());
		assertTrue("failed to get expected number of " + EdgeAttributesElement.ASPECT_NAME + " aspects",
				r0.get(EdgeAttributesElement.ASPECT_NAME).size() == 4);
		...
		assertTrue(ea2.getPropertyOf().size() == 1);
		assertTrue(ea2.getPropertyOf().contains(2L));
		...
		assertTrue(ea3.getPropertyOf().size() == 1);
		assertTrue(ea3.getPropertyOf().contains(3L));
